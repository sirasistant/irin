	...
	CExperiment(pch_name, COLLISION_MODEL_SIMPLE, COLLISION_HANDLER_POSITION)
{
	...
	if (paramsFile == NULL )
	{
		...
		/* Light */
		m_unLightSensorsUsedNumber = CLightSensor::SENSOR_NUMBER;
		m_unLightSensorsUsedValue = new unsigned int[CLightSensor::SENSOR_NUMBER];
		for ( int i = 0 ; i < CLightSensor::SENSOR_NUMBER ; i++)
		{
			m_unLightSensorsUsedValue[i] = 1;
		}
		
		/* Blue Light */
		m_unBlueLightSensorsUsedNumber = CBlueLightSensor::SENSOR_NUMBER;
		m_unBlueLightSensorsUsedValue = new unsigned int[CBlueLightSensor::SENSOR_NUMBER];
		for ( int i = 0 ; i < CBlueLightSensor::SENSOR_NUMBER ; i++)
		{
			m_unBlueLightSensorsUsedValue[i] = 1;
		}
		
		/* Red Light */
		m_unRedLightSensorsUsedNumber = CRedLightSensor::SENSOR_NUMBER;
		m_unRedLightSensorsUsedValue = new unsigned int[CRedLightSensor::SENSOR_NUMBER];
		for ( int i = 0 ; i < CRedLightSensor::SENSOR_NUMBER ; i++)
		{
	...
	else
	{
		...
		/* Get the others */
		for ( int i = 1 ; i < CLightSensor::SENSOR_NUMBER ; i++)
		{
		...
		/* Get the others */
		for ( int i = 1 ; i < CBlueLightSensor::SENSOR_NUMBER ; i++)
		{
		...
		/* Get the others */
		for ( int i = 1 ; i < CRedLightSensor::SENSOR_NUMBER ; i++)
		{
	...
	while(it!=(*vEpucks).end()){                        
		...
		//{
			...
			//{
				...
				//randomX = ( Random::nextDouble() * initAreaRadius * 2.0 ) - initAreaRadius;
				//randomY = ( Random::nextDouble() * initAreaRadius * 2.0 ) - initAreaRadius;
				...
				//}
			//}	
			//while (dist > initAreaRadius || robotsMounted == 1);
		//}
